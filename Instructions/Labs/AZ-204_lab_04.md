...
1.  在 **“导入数据库”** 边栏选项卡中，执行以下操作：
    ...
    > **备注**： 等待数据库创建完成，然后再继续本实验室。如果在导入步骤中收到与防火墙相关的错误，则表示你没有在实验室早些时候在 SQL Server 上正确配置 **“允许 Azure 服务访问服务器”** 设置。  查看你的设置，删除空白 **AdventureWorks** 数据库，然后再次尝试导入。
