...
1.  在 **“标识”** 边栏选项卡中，找到 **“系统分配”** 选项卡，然后执行以下操作：
...
1.  在 **“函数应用”** 边栏选项卡中，选择 **“功能”** 下拉列表旁边的加号 (**+**)。
...
1.  在 **“新建 Azure 函数”** 快速入门中，执行以下操作：
    ...
    1.  在 **“新建函数”** 弹出窗口中，找到 **“名称”** 文本框并输入 **“FileParser”**。
...
1.  选择 **“保存并运行”** 以保存脚本并执行函数的测试。

1.  脚本第一次执行时，将自动显示“测试”和“日志”窗格。

    > **注意**：你可能会在编译期间在日志中注意到警告内容。这可以安全地忽略。

1.  找到“测试”窗格中的 **“输出”** 文本框。你现在应该看到函数返回 **“测试成功”** 值。
...
1.  找到“测试”窗格中的 **“输出”** 文本框。你现在应该看到函数返回的连接字符串。
...
1.  在 **“上传 blob”** 弹出窗口中，执行以下操作：
    ...
    1.  在 **“文件资源管理器”** 窗口中，浏览到 **“Allfiles (F):\\Allfiles\\Labs\\06\\Starter”**，选择 **“records.json”** 文件，然后选择 **“打开”**。
...
1.  在 **“函数应用”** 功能选项卡中，找到并选择现有的 **“FileParser”** 函数以打开函数编辑器。
    ...
    > **注意**：你可能需要在边栏选项卡的菜单中展开 **“函数”** 选项。

1.  在编辑器中，选择 **“查看文件”** 打开选项卡。

1.  在 **“查看文件”** 标签页中，选择 **“添加”**。

1.  在 **“文件名”** 对话框中，输入 **“function.proj”**，然后选择“Enter”，之后将显示一个空的代码编辑器。
...
1.  在编辑器中，选择 **“保存”** 以保留配置更改。
    ...
    > **注意**：这个 .proj 文件包含导入 [Azure.Storage.Blob](https://www.nuget.org/packages/Azure.Storage.Blobs/12.0.0) 包所需的 NuGet 包引用。

1.  选择 **“run.csx”** 文件以返回 **“FileParser”** 函数编辑器。

1.  最小化 **“查看文件”** 选项卡。

    > **备注**：你可以通过选择与选项卡标题相关联的箭头来最小化选项卡。
...
1.  添加以下代码行，通过将 *connectionString* 变量传递给构造函数新建 **BlobServiceClient** 类实例：

    ```
    BlobServiceClient serviceClient = new BlobServiceClient(connectionString);
    ```

1.  添加以下代码行，使用 **BlobServiceClient.GetBlobContainerClient** 方法，并传入 **drop** 容器名称，从而创建一个引用之前在本实验室中所创建容器的 **BlobContainerClient** 类新实例：
...
1.  找到“测试”窗格中的 **“输出”** 文本框。你现在应该看到存储帐户中存储的 **“$/drop/records.json”** blob 内容。
...
1.  如果这是你第一次使用订阅打开 Cloud Shell，你可以使用 **欢迎来到 Azure Cloud Shell 向导** 来配置 Cloud Shell 的初次使用。在向导中执行以下操作：
    ...
    > **注意**：等待 Cloud Shell 完成首次设置过程后，再继续本实验室内容。如果 Cloud Shell 配置选项未显示，这很可能是因为你在本课程实验室中使用的是现有订阅。实验室是在你使用的是新订阅的假设下编写的。
...
1.  在提示符中，键入以下命令，然后按 Enter，找到用于删除资源组的可用命令列表：
